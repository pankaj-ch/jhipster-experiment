...
@RestController
@RequestMapping("/api/hachi-kafka")
public class HachiKafkaResource {

    private final Logger log = LoggerFactory.getLogger(HachiKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public HachiKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
